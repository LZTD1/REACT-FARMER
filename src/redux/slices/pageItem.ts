...
import { IPageItem } from '../../@types/redux/IPageItem';

const initialState: IPageItem = {
  item: null,
  status: { state: 'pending', meta: '' },
};

export const fetchItemById = createAsyncThunk<IPageItem['item'], string>(
  'pageItem/fetchItemById',
  async (fetchedID, thunkAPI) => {
    const res = await axios.get<IPageItem['item']>(
